...
const StoriesList = ({ stories, activeFilter, ads, searchTerm }) => {
  ...
  const addSearchFilter = (list) => {
    return list.filter(story => story.data.title.toLowerCase().includes(searchTerm.toLowerCase()))
  }
        ...
        return (
          <div key={chunkIndex}>
            {chunk.map((story) => {
              ...
              return (
                <div key={story.uid} className={cx({[styles.Show]: story.data.title.toLowerCase().includes(searchTerm.toLowerCase()) || !searchTerm || searchTerm.length < 1})}>
